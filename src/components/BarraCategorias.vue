<template>
    ...
    <div style="position: sticky; top: 3.5rem; z-index: 999; background-color:black;color: white; overflow-x: auto;" class="col-12 nav_bar shadow-3 d-flex  lg:justify-content-start align-items-center mb-5 p-0 md:p-0">
        ...
        <div v-for="(section,index) in cart?.menu?.listaCategorias?.filter(c => codigos.includes(parseInt(c.categoria_id) ))" :key="section.categoria_id" class="p-0"  style="display: flex; align-items: center;">
            ...
            <Button    size="small" @click="navigateToCategory(section.categoria_descripcion,section.categoria_id)" :label="section.categoria_descripcion "
                ...
                class="py-3 px-3 mx-0  text-lg titulo" style=" text-transform: capitalize;width:max-content;text-transform: uppercase; color:white; border-radius: 0;outline: none;box-shadow:color none;font-weight: bold;">
               
            </Button>
            <span style="color: white;" v-if="index < cart?.menu?.listaCategorias?.filter(c => codigos.includes(parseInt(c.categoria_id))).length-1"> <b>| </b></span>
        </div>
    </div>
    </div>
